...
class Cache
{
    ...
    */
    function get($filename = NULL, $use_expires = true)
    {
        ...
        if ($use_expires && ! empty($this->_contents['__cache_expires']) 
                && $this->_contents['__cache_expires'] < time() ) {
        ...
        if (isset($this->_contents['__cache_dependencies'])) {
            foreach ($this->_contents['__cache_dependencies'] as $dep) {
                ...
                if (! file_exists($this->_path.$dep.'.cache') 
                        or filemtime($this->_path.$dep.'.cache') > $cache_created) {
                    $this->delete($filename);
                    return FALSE;
                }
            }
